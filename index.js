...
async function main() {
  ...
  try {
    // Step 0-a: 店舗マスタを公式APIと同期する（新店の自動追加・改称の自動追随）
    // 週1回のPlaywright走査だけに任せていた頃は、新店が最大7日レポートに出ず、
    // 改称にも追随できなかった（吉祥寺が4日間「新宿」として報告された）。
    // データ取得の前に回すことで、開店・改称の翌朝から正しい店舗名で出る。
    // 失敗しても既存マスタで続行する（フェイルオープン。ここで日次を止めない）。
    try {
      run('sync-stores-master.js', '店舗マスタ同期（公式API）');
    } catch (err) {
      log(`⚠️ 店舗マスタ同期に失敗（既存マスタで続行）: ${err.message}`);
    }
